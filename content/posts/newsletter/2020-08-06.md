...
Pep. [wrote an article][https://bouah.net/2020/07/what-about-design/]
detailing his thoughts on XMPP and on the importance of design.
...
The multi-platform desktop client [Gajim 1.2.1](https://gajim.org/post/2020-07-08-gajim-1.2.1-released/) has been released. Just two weeks after the release of Gajim 1.2, the next update is already there. Gajim 1.2.1 features spam reporting as specified by [XEP-0377](https://xmpp.org/extensions/xep-0377.html), automatic activation of shipped plugins which implies OMEMO available by default, and some bug fixes. Audio/Video calls gain some improvements, but remain highly experimental. [More development news from July](https://gajim.org/post/2020-07-26-development-news-july/): an automatic update check for Gajim on Wndows/MacOS, account password storage enhancements, an OMEMO fix for ChatSecure, and many small improvements.
...
The [git repository of Pix-Art Messenger](https://codeberg.org/kriztan/Pix-Art-Messenger/src/branch/master/CHANGELOG.md) moves from Github to Codeberg. This process is not finished yet, 
until then both repositories will run in parallel. The integrated update service already downloads update files from Codeberg releases.
Furthermore, Pix-Art Messenger versions 2.5.1 and 2.5.2 have been released, which for example highlight pinned chats in color. 
Furthermore, the video quality has been improved a bit. Large files are compressed more, smaller ones are compressed a bit less. This 
process is automated and tries to calculate the file size after compression to set the compression to the maximum file size for upload.
...
[Tigase XMPP Server 8.1.0 General Availability has been released!](https://tigase.net/tigase-xmpp-server-8.1.0-ga/)
Tigase #XMPP Server 8.1.0 GA has been released and it's packed with improvements! 
`SASL-EXTERNAL` mechanism defined in [XEP-0178: Best Practices for Use of SASL EXTERNAL with Certificates](https://xmpp.org/extensions/xep-0178.html) was added for server-to-server (federated, s2s) connections greatly improving compliance with XMPP network. It’s possible to use both SASL-EXTERNAL and Diallback depending on support in other servers. Set of XMPP extensions were added: [XEP-0398](https://xmpp.org/extensions/xep-0398.html): User Avatar to vCard-Based Avatars Conversion, [XEP-0156](https://xmpp.org/extensions/xep-0156.html): Discovering Alternative XMPP Connection Methods - Tigase already supported handling DNS queries and standardised our `webservice` to XEP-0156, [XEP-0410](https://xmpp.org/extensions/xep-0410.html): MUC Self-Ping (Schrödinger’s Chat), [XEP-0153](https://xmpp.org/extensions/xep-0153.html): vCard-Based Avatars - added support for setting **vCard avatar for MUC rooms**, [XEP-0411](https://xmpp.org/extensions/xep-0411.html): Bookmarks Conversion, [XEP-0157](https://xmpp.org/extensions/xep-0157.html): Contact Addresses for XMPP Services that can be configured on per VHost basis. This version also improves management of multiple domains VHost - there are even more settings that can be configured on per-domain basis and also define defaults for all VHosts. There is also new anti-spam plugin enabled by default.
